...
class PlatformGithub extends Component<{
    type: string;
    ...
    onTokenChecking = () => {
        ...
        GITHUB.checkRepos(token).then((response: any) => {

            if (response.status === 200) {
                const repos = response.data.map((item: any) => {
                    return { jsDelivr: true, token: token, id: item.id, name: item.name, owner: { login: item.owner.login, id: item.owner.id, avatar_url: item.owner.avatar_url }, private: item.private, permissions: item.permissions }
                })
                this.setState({
                    repos: repos,
    ...
    render() {
        ...
        return (
            <>
                <Form.Item extra={<><a style={{fontSize: '12px'}} href="https://github.com/settings/tokens" target="_blank">{t("settings.getAccessToken")}</a><a style={{fontSize: '12px', float: 'right', lineHeight: '24px'}} href="https://mp.weixin.qq.com/s?__biz=MzU3MDMyNDk5OQ==&mid=2247484407&idx=1&sn=ab0f37ce346bc4f3bc9ca4a9544fa0e9&chksm=fcf0629fcb87eb890a3939b2d9579bdb1bd6a14cc234bca37c9b799f59090031bbd316aea341&token=1201531960&lang=zh_CN#rd" target="_blank"><QuestionCircleOutlined style={{marginRight: '5px'}}/>{t("settings.guide")}</a></>} name={`${type}_token`} label={t("settings.accessToken")} rules={[{ required: true, message: '请填写AccessToken' }]} validateStatus={tokenChecked ? 'success' : 'error'}>
                ...
                <Form.Item name={`${type}_repo`} extra={type === 'image' && <span style={{fontSize: '12px'}}>{t("settings.imageHostingTips")}{t("settings.githubTips")}</span>} label={t("settings.repository")} rules={[{ required: true, message: t("settings.repositoryRequired") }]}>
                    <Select
                        showSearch
                        allowClear
                        style={{ width: '100%' }}
...
export default withTranslation()(PlatformGithub)